...
Deno.test("createNoopClog - returns first argument as string", () => {
...
Deno.test("createNoopClog - callable interface returns first arg", () => {
	const clog = createNoopClog("test");
...
Deno.test("createNoopClog - does not call any writer or console", () => {
	reset();
	const clog = createNoopClog("test");
...
Deno.test("createNoopClog - does not trigger global hook", () => {
	reset();
	capturedData = [];
	createClog.global.hook = (data: LogData) => {
		capturedData.push(data);
...
Deno.test("createNoopClog - ns property works", () => {
...
Deno.test("createNoopClog - ns property is readonly", () => {
	const clog = createNoopClog("test");
...
Deno.test("createNoopClog - return value works for throw pattern", () => {
	const clog = createNoopClog("test");
...
Deno.test("stacktrace - global flag appends stack trace", () => {
	reset();
...
Deno.test("stacktrace - per-instance config works", () => {
	reset();
...
Deno.test("stacktrace - instance true overrides global false", () => {
	reset();
	...
	assertEquals(consoleOutput.log.length, 1);
	assert(consoleOutput.log[0].includes("at "), "Stack trace should be present");

	restoreConsole();
});

Deno.test("stacktrace - instance false overrides global true", () => {
	reset();
	createClog.global.stacktrace = true;
	...
	assert(!consoleOutput.log[0].includes("at "), "Stack trace should not be present");

	restoreConsole();
});

Deno.test("stacktrace - number limits frame count", () => {
	reset();
...
Deno.test("stacktrace - works with JSON output", () => {
	reset();
	createClog.global.jsonOutput = true;
	createClog.global.stacktrace = true;
...
Deno.test("stacktrace - works with concat mode", () => {
	reset();
	createClog.global.concat = true;
	createClog.global.stacktrace = true;
...
Deno.test("stacktrace - works with all log levels", () => {
	reset();
	createClog.global.stacktrace = true;
